> **Update (2026-09-07):** Revisited this project and fixed a bug in the potential-energy calculations that was producing inaccurate dynamics, most visibly benzene's ring bending out of its planar shape instead of staying rigid. As part of that fix, all bond/angle/torsion constants in `constants.py` were switched from Lifson & Warshel to AMBER GAFF (`gaff.dat`). The original version of this repo remains available at [`5fd783c`](https://github.com/f4t4nt/molecular-simulation/tree/5fd783c7d19ee6d77d2b2c405a6d21602bd6cb93).

# Hydrocarbon Molecular Simulation

We developed a program to simulate the dynamics of hydrocarbon molecules. The original methodology used the consistent force field method devised by Shneior Lifson and Arieh Warshel; as of the update above, force constants are instead sourced from AMBER GAFF.
...
- Current force field constants source: J. Wang et al., ["Development and testing of a general amber force field"](https://doi.org/10.1002/jcc.20035), J. Comput. Chem. 25, 1157 (2004)
